...
const { generateQuestionsForTrends } = require("./generateQuestions.cjs");
const { fetchTrendingTopics } = require("./fetchTrends.cjs");
const fs = require("fs");
...
// Fallback trending topics (used if API fetch fails)
...
async function deployMarket(factory, question, duration, signer) {
    ...
    try {
        ...
        const parsedLogs = receipt.logs
            ...
            .filter(log => log && log.name === "MarketCreated");
        
        if (parsedLogs.length > 0) {
            marketAddress = parsedLogs[0].args[0];
        } else {
        ...
        // Last resort: get latest market from factory
        const allMarkets = await factory.getAllMarkets();
        marketAddress = allMarkets[allMarkets.length - 1];
    }

    if (!marketAddress) {
        throw new Error("Failed to get market address from deployment");
    }
...
async function main() {
    ...
    console.log("\n" + "=".repeat(60));
    console.log("🎯 Fetching Trending Topics & Generating Prediction Markets\n");

    // Fetch trending topics (or use fallback)
    let trendingTopics;
    try {
        ...
        console.log(`⚠️  Failed to fetch trends: ${error.message}`);
        console.log("📋 Using fallback topics...");
        trendingTopics = FALLBACK_TOPICS;
    }

    // Generate questions from trending topics
    const questions = generateQuestionsForTrends(trendingTopics);
    console.log(`📝 Generated ${questions.length} prediction questions\n`);
    ...
    console.log("=".repeat(60));
    if (deploymentData.markets.length === 0) {
        ...
        deploymentData.markets.forEach((market, index) => {
            ...
            console.log(`   Duration: ${Math.floor(market.duration / 60)} minutes`);
            console.log(`   Liquidity: ${market.initialLiquidity.yes} ETH (YES) / ${market.initialLiquidity.no} ETH (NO)`);
        });
    }
